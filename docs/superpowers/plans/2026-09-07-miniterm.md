...
vitest.config.ts                  node ortamı, svelte eklentisi

src/main.ts                       uygulama bootstrap
src/App.svelte                    kabuk: sidebar + ana alan + ayarlar görünümü + toast
...
src/store/app.svelte.ts           runes tabanlı uygulama durumu (ağaç, seçim, görünüm)
src/store/sidebar.ts              ağaç düzleştirme, silme uyarısı, yeni klasör adı
src/store/dnd.ts                  sürükle-bırak bölge hesabı ve hedef çözümleme
src/store/grid.ts                 hücre listesi, ayırıcı sayısı, grid-template üretimi
src/store/wizard.ts               sihirbaz taslağı ve taslaktan Workspace üretimi
src/store/settings.ts             son kullanılanlar, komut önizlemesi, araç/dizin CRUD
src/store/lru.ts                  saf LRU touch/evict
src/store/sessions.svelte.ts      oturum durumu, canlı instance LRU orkestrasyonu

src/term/exit.ts                  process-exit bildirim metni
src/term/theme.ts                 xterm font ve renk teması

src/lib/Sidebar.svelte            ağaç kabuğu, başlık, + butonu
src/lib/TreeItem.svelte           tek satır: girinti, ikon, rename, sürükle-bırak, durum
src/lib/ContextMenu.svelte        sağ tık menüsü
src/lib/TerminalGrid.svelte       rows x cols yerleşim + ayırıcılar
...
src-tauri/src/pty/batch.rs        çıktı toplayıcı (8 ms / 32 KB)
...
Task 15'te yazılan `RECENT_LIMIT` / `pushRecent` kalır. `import` satırı dosyanın **en üstüne**, geri kalanı `pushRecent`'in altına gider:
